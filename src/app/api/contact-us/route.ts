...
type RequestBodyTyp = { name: string; email: string; message: string };
export async function POST(req: Request) {
    const { name, email, message } = (await req.json()) as RequestBodyTyp;
    ...
    const msg = {
        ...
        subject: "Contact Us Request from KEEP ME POSTED",
        text: `
        Name: ${name}\n
        Email: ${email} \n
        Message: ${message}
